...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 Checking endpoint: method=${req.method}, endpoint="${endpoint}", fileId=${fileIdParam || 'null'}`);
    if (req.method === 'GET' && endpoint === 'fixed-expenses') {
      ...
      console.log(`📊 Using API Key: ${googleApiKey.substring(0, 20)}...`);
      ...
      // Tek tırnak içinde tek tırnak varsa çift tırnak yap
      ...
      try {
        // Google Sheets API values endpoint (API key ile)
        ...
        console.log(`📊 Google Sheets API response status: ${sheetsResp.status}`);

        // Eğer 404 hatası alırsak, alternatif sheet adlarını deneyelim
        if (!sheetsResp.ok && sheetsResp.status === 404) {
          console.warn(`⚠️ Sheet "${sheetName}" bulunamadı, alternatif sheet adlarını deniyorum...`);
          
          // Alternatif sheet adları: Sheet1, Sheet 1, aylik_gider_duzeni, Aylık Gider Düzeni
          const alternativeSheetNames = ['Sheet1', 'Sheet 1', 'aylik_gider_duzeni', 'Aylık Gider Düzeni', 'Aylik Gider Duzeni'];
          
          for (const altSheetName of alternativeSheetNames) {
            if (altSheetName === sheetName) continue; // Zaten denedik
        ...
        if (!sheetsResp.ok) {
          ...
          try {
            const errorJson = JSON.parse(errorText);
            if (errorJson.error) {
              const errorMessage = errorJson.error.message || String(errorJson.error);
              const errorCode = errorJson.error.code || '';
              errorDetail = `Error: ${errorMessage} (code: ${errorCode})`;
              
              // "This operation is not supported for this document" hatası için özel mesaj
              if (errorMessage.includes('not supported for this document') || errorCode === 'FAILED_PRECONDITION') {
                errorDetail += '\n\n🔧 ÇÖZÜM:\n';
                errorDetail += 'Bu hata dosyanın Excel formatında olduğunu gösterir. Google Sheets formatına dönüştürmeniz gerekiyor.\n\n';
                errorDetail += '1. Google Drive\'da dosyayı açın: https://docs.google.com/spreadsheets/d/1_M2g7x4DQs8OQuZzrk4qWkLTMFGRrd-1/edit\n';
                errorDetail += '2. File > Save as Google Sheets yapın\n';
                errorDetail += '3. VEYA "Open with" > "Google Sheets" seçin ve yeni dosya olarak kaydedin\n';
                ...
                errorDetail += '⚠️ NOT: Excel dosyaları (.xlsx) Google Sheets API ile okunamaz. Dosyanın native Google Sheets formatında olması gerekir.';
                  ...
                  errorMessage.includes('Permission') ||
                  sheetsResp.status === 403) {
                isPermissionError = true;
                errorDetail += '\n\n🔒 ÇÖZÜM:\n';
                errorDetail += '1. Google Sheets dosyasını kontrol edin: https://docs.google.com/spreadsheets/d/1_M2g7x4DQs8OQuZzrk4qWkLTMFGRrd-1/edit\n';
                errorDetail += '2. Dosyanın "Herkes linki olan herkes görüntüleyebilir" veya OAuth hesabınızın erişim izni olduğundan emin olun\n';
                errorDetail += '3. OAuth token\'ın şu scope\'lara sahip olduğundan emin olun:\n';
                errorDetail += '   - https://www.googleapis.com/auth/drive.readonly\n';
                errorDetail += '   - https://www.googleapis.com/auth/spreadsheets.readonly\n';
                errorDetail += '4. Refresh token\'ı yeniden alın (yukarıdaki scope\'lar ile)';
              }
            }
          } catch (_) {
            // JSON parse edilemezse text'i kullan
            if (errorText.includes('PERMISSION_DENIED') || errorText.includes('permission') || sheetsResp.status === 403) {
              isPermissionError = true;
              errorDetail += '\n\n🔒 Google Sheets dosyasına erişim izni yok. Lütfen dosya paylaşım ayarlarını kontrol edin.';
            }
          }
          
          return new Response(JSON.stringify({ 
            error: isPermissionError ? 'Erişim izni hatası' : 'Google Sheets okunamadı', 
            detail: errorDetail, 
